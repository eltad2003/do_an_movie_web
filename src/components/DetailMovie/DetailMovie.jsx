...
const DetailMovie = () => {
    ...
    const type = { generes: 'Thể loại', content: 'Nội dung', year: 'Năm phát hành', country: 'Quốc gia', status: 'Trạng thái', actors: ' Diễn viên', director: 'Đạo diễn' }

    const fetchDetailMovie = async () => {
        try {
            setIsLoading(true)
            const res = await fetch(`${import.meta.env.VITE_API_URL}/phim/${slug}`)
            if (!res.ok) {
